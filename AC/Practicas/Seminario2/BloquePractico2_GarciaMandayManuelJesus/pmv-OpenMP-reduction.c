...
int main(int argc, char **argv) {
	...
	for(i = 0; i < n; i++){
		v1[i] = (rand() % 3) + 1;
		v3[i] = 0;
		#pragma omp parallel for shared(i) private(j) shared(n) shared(m) default(none)
		for(j = 0; j < n; j++)
			m[i][j] = rand() % (3+1);
	}
	...
	for(i = 0; i < n; i++){
		#pragma omp parallel for shared(i) private(j) shared(n) shared(m) shared(v1) shared(v3) reduction(+:suma) default(none)
		for(j = 0; j < n; j++){
			suma = m[i][j] * v1[j];
		}
