...
# Abro la bases de datos para lectura y escritura, si no exisitiera me la crearía (todo por el flag c) 
db = dbm.open('cache', 'c')
...
# funcion para inicializar la variable con todas las opciones del menu
...
class classlogout:
	def GET(self):	
		...
		if acceso == 1:
			...
			comprueba_sesion()
			varMakoaux = [variableMako[0], variableMako[1]]
...
class classbusqueda:
	...
	def POST(self):
		...
		if not formulario_busqueda.validates():
			...
			if (pas in db.keys()):
				res = db[pas]
				varMakoaux.append("Nombre: " + str(res))
			else:
